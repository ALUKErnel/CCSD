...
from data_prep.xstance_dataset_cross import get_datasets_main
...
fh = logging.FileHandler(os.path.join(opt.model_save_file, 'prompt_cross_kl_log.txt'))
log.addHandler(fh)
...
def train(opt):
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i, (inputs,inputs_cross, labels, labels_cross, y) in tqdm(enumerate(train_iter), total=max_iter_per_epoch):
            ...
            outputs = model(**tokenized_inputs, labels=labels_compute)
            ...
            probs = torch.vstack((probs_favor, probs_against)).reshape(-1,2)


            # cross 
            tokenized_inputs_cross = tokenizer(inputs_cross, padding=True, return_tensors="pt").to(opt.device)
            tokenized_labels_cross = tokenizer(labels_cross, padding=True, return_tensors="pt")["input_ids"].to(opt.device)
            labels_compute_cross = torch.where(tokenized_inputs_cross.input_ids == tokenizer.mask_token_id, tokenized_labels_cross, -100)
            mask_token_index_cross = (tokenized_inputs_cross.input_ids == tokenizer.mask_token_id).nonzero(as_tuple=True)
            outputs_cross = model(**tokenized_inputs_cross, labels=labels_compute_cross)
            ce_loss_cross = outputs_cross.loss

            logits_cross = outputs_cross.logits
            probs_favor_cross = logits_cross[mask_token_index_cross[0], mask_token_index_cross[1]][:, favor_id]
            probs_against_cross = logits_cross[mask_token_index_cross[0], mask_token_index_cross[1]][:, against_id]
            probs_cross = torch.vstack((probs_favor_cross, probs_against_cross)).reshape(-1,2)

            # consistency kl divergence
            kl_loss_1 = F.kl_div(probs.log_softmax(dim=-1), probs_cross.softmax(dim=-1))
            kl_loss_2 = F.kl_div(probs_cross.log_softmax(dim=-1), probs.softmax(dim=-1))

            kl_loss = kl_loss_1 + kl_loss_2

            final_loss = ce_loss + ce_loss_cross + kl_loss
    ...
    torch.save(best_model.state_dict(), '{}/prompt-fine-tuned-model.pth'.format(opt.model_save_file)) 
...
def evaluate(opt, data_loader, model):
    ...
    with torch.no_grad():
        for inputs, inputs_cross, labels, labels_cross, y in tqdm(iter1):
